...
const server = http.createServer(async function (req, res) {
  ...
  if (urlsplit[1] == "botcheck") {
    if (urlsplit[2] == "create" && req.method == "POST") {
      (async function () {
        try {
          var generated = await botCheck.createRobotTest();
          res.end(
            JSON.stringify({
              id: generated.id,
            })
    ...
    if (urlsplit[2] == "image" && req.method == "GET") {
      var id = urlsplit[3];
      (async function () {
        await botCheck.serveRobotTestImage(id, res);
      })();
      return;
    }
    if (urlsplit[2] == "check" && req.method == "POST") {
      var id = urlsplit[3];
      (async function () {
        var body = await waitForBody(req);
        try {
          var json = await JSON.parse(body.toString());
        } catch (e) {
          res.statusCode = 400;
        ...
        if (typeof json.code !== "string") {
          res.statusCode = 400;
          res.end("code property must be string.");
          return;
        }
        var didPass = botCheck.sendCheck(id, json.code);
        res.end(
          JSON.stringify({
            passed: didPass,
          })
